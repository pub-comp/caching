...
namespace PubComp.Caching.Core
{
    public class CacheConfigurationHandler : IConfigurationSectionHandler
    {
        ...
        {
            var configuration = new List<CacheConfig>();
            ...
            foreach (XmlNode child in section.ChildNodes)
            {
                ...
                {
                    configuration.Add(new NoCacheConfig { Action = ConfigAction.Remove, Name = nameNode.Value });
                    continue;
                }
                ...
                if (configType.IsSubclassOf(typeof(CacheConfig)) == false)
                    continue;

                var cacheConfig = configType.GetConstructor(new Type[0]).Invoke(new object[0]) as CacheConfig;
                if (cacheConfig == null)
                    continue;
