...
{
    [TestClass]
    public class LayeredScopedCacheTests
    {
        protected IMockCache GetMockCache(string name)
        {
            return new MockMemScopedCache(name);
        }

        protected ICache GetLayeredCache(string name, ICache level1, ICache level2)
        {
            return new LayeredScopedCache(name, level1, level2);
        }

        protected ICache GetLayeredCache(string name, string level1CacheName, string level2CacheName)
        {
